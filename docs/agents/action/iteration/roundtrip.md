...
- While you are actively debugging one mode, use the focused loops instead: [Compose Loop](compose.md) for the compose deploy, [Swarm Loop](swarm.md) plus the `act-swarm-*` targets for the swarm deploy. Return to the roundtrip once both modes pass in isolation.
