...
├── tests/                       # DinD E2E (see below)
│   ├── e2e.sh                   # orchestrator (replaces example.sh.example)
...
│   └── test.env.j2              # env contract sourced by the orchestrator
...
`roles/svc-net-wireguard/example.sh.example` is replaced by the real `tests/` harness above.
...
- [x] `roles/svc-net-wireguard/tests/` contains an orchestrator (`e2e.sh`) that mirrors the structure of `example.sh.example` (svc-runner pattern): it env-gates required variables, verifies the backend, then delegates to `local.sh` and `external.sh`. The `example.sh.example` stub is removed. (Scripts are shellcheck-clean.)
...
- [x] The orchestrator is structured so the instance-provisioning backend is swappable via `WIREGUARD_E2E_BACKEND` (Compose today; Swarm / Kubernetes later) without changing `external.sh`. Documented in the `e2e.sh` header.

### Full mesh across all servers and clients

- [x] `tests/mesh.sh` exists, is wired into `e2e.sh` (runs after `local.sh` / `external.sh`), and runs entirely in Docker-in-Docker alongside the 3-server stage (DinD logic preserved). (Shellcheck-clean.)
...
- `shellcheck` clean (via `make test`) on `tests/e2e.sh`, `tests/local.sh`, `tests/external.sh`, `tests/mesh.sh`.
...
- The DinD `tests/e2e.sh` run — ≥3 servers + handshake (`local.sh` / `external.sh`) and the
...
bash roles/svc-net-wireguard/tests/e2e.sh
```
...
5. Build the DinD multi-server E2E harness (`e2e.sh` / `local.sh` / `external.sh` / `test.env.j2`) mirroring `example.sh.example`; remove the stub.
