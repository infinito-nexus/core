...
The repository snapshot evaluated for this requirement contains 169 roles with a literal `application_id` and 18 role-local `mcp:` blocks.
The audit below classifies every one of those 169 roles exactly once.
...
Implementation MUST replace the snapshot check with a test that derives the current set dynamically; it MUST NOT hard-code the number 169.
...
### Authorization enabler or subordinate integration (3)

`web-app-keycloak`, `web-svc-collabora`, `web-svc-onlyoffice`.

### No shared application MCP surface (94)
...
**`privileged_control_plane` (22):** `svc-bkp-local-2-device`, `svc-bkp-nfs-2-local`, `svc-bkp-remote-2-local`, `svc-bkp-secrets-2-local`, `svc-bkp-volume-2-local`, `svc-dns-unbound`, `svc-net-wireguard-core`, `svc-net-wireguard-firewalled`, `svc-net-wireguard-plain`, `svc-opt-keyboard-color`, `svc-opt-ssd-hdd`, `svc-opt-swapfile`, `svc-prx-openresty`, `svc-registry-cache`, `svc-registry-docker`, `svc-runner`, `svc-storage-nfs-client`, `svc-storage-nfs-server`, `svc-swarm-manager`, `svc-swarm-node`, `svc-virt-kata`, `update`.
...
**`no_remote_surface` (23):** `svc-ai-ollama`, `svc-ai-robot`, `web-app-chess`, `web-app-dashboard`, `web-app-fediwall`, `web-app-hugo`, `web-app-littlejs`, `web-app-mig`, `web-app-mini-qr`, `web-app-navigator`, `web-app-roulette-wheel`, `web-app-sphinx`, `web-opt-rdr-domains`, `web-opt-rdr-www`, `web-svc-asset`, `web-svc-cdn`, `web-svc-coturn`, `web-svc-css`, `web-svc-file`, `web-svc-html`, `web-svc-legal`, `web-svc-logout`, `web-svc-simpleicons`.
...
- [ ] Lint rejects every generic URL/spec/query/SQL/filesystem/shell/socket/unrestricted-bucket surface described in this requirement.
...
- [ ] `mcp_servers` resolves the declared credential owner and source rather than `users.administrator.tokens`, and deployment fails for missing, empty, shared, or overprivileged credentials.
- [ ] `auth_subject` is validated against the identity actually provisioned in the provider and is not documentation-only.
- [ ] `auth: oidc` is rejected by static client renderers until a real acquisition, refresh, audience, expiry, and revocation flow is implemented and tested.
- [ ] Open WebUI grants each server to only `roles/<provider-application-id>/mcp`; one-of-many, no-membership, wrong-app, removal, and last-group removal tests pass.
- [ ] The declarative user configuration can assign `mcp` to one application without assigning it to any other application.
...
- [ ] Providers declare explicit allowed consumers, clients declare supported transports/auth schemes, and discovery computes the intersection without naming peer roles in `group_names` expressions.
- [ ] Every rejected discovery entry has a stable, tested reason; missing credentials, transport mismatch, and unreachable endpoints fail reconciliation when the consumer was explicitly authorized.
...
- [ ] Reconciliation owns only deterministic `infinito:<provider-application-id>` resources, fails on duplicates, and preserves human-created resources.
...
- [ ] Open WebUI with `ENABLE_PERSISTENT_CONFIG=false` restores the exact desired connection state and group grants after a process restart without a full reinstall.
