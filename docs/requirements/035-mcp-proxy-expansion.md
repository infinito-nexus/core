...
- [x] The operational metadata remains role-local, while the generated audit report includes deployed version, authoritative source, transport, authentication, real credential subject, read/write scope, tool allowlist, proposed implementation, selected consumers, and blocker. [audit.md](../contributing/design/role/services/mcp/audit.md) is rendered by [audit.gen.py](../contributing/design/role/services/mcp/audit.gen.py), which only reads `meta/mcp.yml` and never writes back into a role; [test_mcp_audit_report.py](../../tests/lint/repository/documentation/test_mcp_audit_report.py) imports the generator and fails when the committed report differs from a fresh run.
...
- [ ] Compose and Swarm tests prove endpoint DNS/routing from each client and prove an adapter cannot reach unrelated application networks. **The isolation half already runs on every deploy:** `svc-ai-mcp-adapter/tasks/probe.yml` includes `isolation.yml` whenever `mcp.implementation` is `adapter`, and that task reads the networks the sidecar is actually attached to and fails unless they are the role's own subnet plus the platform and declared upstream ones. `test_mcp_provider_probe.py` holds the include, so all eight enabled providers reach it — a provider that skipped the probe would also skip the isolation proof. What remains is the routing half: DNS resolution of each endpoint from each client, in both deploy modes, which only a running cluster can show.
...
- [x] n8n 1.95.3 is declared as an SSE client and server, and its managed credentials and workflows are idempotently provisioned through supported public APIs. `meta/mcp.yml` declares `direction: both` and `transport: sse`; `files/python/provision/mcp.py` renders the server workflow and `files/python/provision/client.py` the agent workflow, both through `/api/v1/workflows`. Credentials go through `/rest` because the public API of that tag exposes only create and delete for them, which cannot converge an existing one. Two idempotence defects were fixed here: the run reported `CHANGED` forever, because the public-API key it minted was found again on the next run and counted as a change, and that key was left standing in n8n afterwards although nothing read it. It is now minted per run, revoked before the run ends, and no longer reported.
