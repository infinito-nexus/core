...
- **CI images are built per architecture and merged into one tag.** The build is
  native on each architecture's own runner rather than emulated, so
  [push.sh](../../../scripts/image/push.sh) pushes `<tag>-amd64` and
  `<tag>-arm64` and
  [manifest_all.sh](../../../scripts/image/manifest_all.sh) publishes the plain
  `<tag>` as a manifest list. Every consumer keeps pulling the plain tag and
  gets the image matching its machine.
...
flowchart LR
    ...
    B["images build<br/>one job per architecture"] --> M["manifest list<br/>one tag, both machines"]
    M --> R
```
...
- The CI image build doubles in job count. It does not double in wall clock,
  because the two architectures build in parallel on their own runners, and each
  keeps its own build cache tag.
...
| Build arm64 images by QEMU emulation on amd64 runners | One build host, but every arm64 image is built through emulation, and a native runner exists for the same cost. |
