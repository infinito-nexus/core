...
| [test-run.yml](../../../../../.github/workflows/test-run.yml): `🏃 Test: Run` | Extracts the `### Production` command from a role README (default `svc-db-postgres`) and executes it against the runner: `<your-server>` becomes `localhost` (ansible_connection local) and the Docker socket replaces the TTY flag. Verifies the documented one-command deploy actually works. Runs in the orchestrator parallel to `test-environment.yml` and gates the final `done` job. | reusable, manual | `role` (default `svc-db-postgres`) |
