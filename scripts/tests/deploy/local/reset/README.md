...
|---|---|---|---|
| `inventory.sh` | Recreates `devices.yml` for all discovered apps without deploying them. | `INFINITO_DISTRO`, `INFINITO_INVENTORY_DIR` | Rebuilds the local inventory and keeps the runtime `.password` file. |
