...
while read -r unit; do
	...
	if ! systemctl start "${unit}"; then
		...
		journalctl -u "${unit}" --no-pager -n 60 2>/dev/null || true # nocheck: shell-or-true -- grandfathered: worked in practice; TODO: sharpen to catch only the exact tolerated error
		exit 1
	fi
done <<<"${units}"
