...
The `main.yml` file in the `dsk-docker` role consists of two primary tasks:
...
cd core
make onboard
make compose-deploy mode=reinstall apps=dsk-docker full_cycle=false
```
...
2. Navigate to the `roles/dsk-docker` directory.
