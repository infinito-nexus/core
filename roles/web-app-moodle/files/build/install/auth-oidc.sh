#!/bin/bash
# Install the Microsoft auth_oidc plugin. All values come from env vars
...
: "${MOODLE_OIDC_PLUGIN_ARCHIVE_URL:?required}"
...
curl --connect-timeout 5 --max-time 300 --retry 3 --retry-all-errors --retry-delay 2 -fSL -o "${ZIP_PATH}" "${MOODLE_OIDC_PLUGIN_ARCHIVE_URL}"
