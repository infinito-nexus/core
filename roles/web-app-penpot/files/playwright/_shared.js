...
async function penpotOidcLogin(page, username, password) {
  ...
  await oidcEntry.click({ timeout: resolveTimeout(30_000) });
  await expect
...
async function penpotLdapLogin(page, email, password) {
  ...
  await ldapButton.click({ timeout: resolveTimeout(30_000) });
  await assertAuthenticated(page);
}
...
async function penpotRegister(page, fullname, email, password) {
  ...
  await next.click({ timeout: resolveTimeout(30_000) });
  ...
  if (await nameField.isVisible({ timeout: resolveTimeout(15_000) }).catch(() => false)) {
    ...
    await finish.click({ timeout: resolveTimeout(30_000) });
  }
  await assertAuthenticated(page);
}
...
async function penpotNativeLogin(page, email, password) {
  ...
  await expect(loginButton, "Expected the login submit button to enable once the form is filled").toBeEnabled({ timeout: resolveTimeout(30_000) });
  await loginButton.click({ timeout: resolveTimeout(30_000) });
  await assertAuthenticated(page);
}
