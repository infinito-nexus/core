...
validator accepts `svc-docker-swarm-manager` as a legitimate
application_id. The actual swarm-init logic lives in `svc-docker-swarm`
and dispatches on `'svc-docker-swarm-manager' in group_names`.

## Features

- **Marker role:** Carries no tasks; selects which `svc-docker-swarm`
