...
if [ -w "${CACERTS}" ]; then
	"${KEYTOOL}" -importcert -noprompt -trustcacerts -alias "${CA_ALIAS}" \
		-file "${CA_FILE}" -keystore "${CACERTS}" -storepass changeit >/dev/null 2>&1 || true  # nocheck: shell-or-true -- grandfathered: worked in practice; TODO: sharpen to catch only the exact tolerated error
