...
test("mautrix-signal addon: bridge appservice bot is provisioned and reachable on the Synapse homeserver", async ({ request }) => {
  ...
  const botResponse = await request.get(profileUrl(botUserId), { failOnStatusCode: false, timeout: resolveTimeout(30_000) });
  expect(
    botResponse.status(),
  ...
  const absentResponse = await request.get(profileUrl(absentBotUserId), { failOnStatusCode: false, timeout: resolveTimeout(30_000) });
  expect(
