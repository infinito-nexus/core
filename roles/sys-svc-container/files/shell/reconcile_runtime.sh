...
if [ "${#reaped[@]}" -gt 0 ]; then
	timeout "${RESTART_TIMEOUT}" systemctl restart docker.service || true # nocheck: shell-or-true -- a restart rc says nothing about dockerd being up; the is-active poll below is the authority and fatals
