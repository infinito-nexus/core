...
command -v docker >/dev/null || { report "docker is not installed, nothing to convert"; exit 0; } # nocheck: raw-docker - runs on a bare host to verify a btrfs layout, where the wrapper does not exist

ROOT="$(docker info --format '{{.DockerRootDir}}' 2>/dev/null || true)" # nocheck: raw-docker - see above
[[ -n "${ROOT}" && -d "${ROOT}" ]] || { report "the docker data root could not be resolved"; exit 0; }
...
command -v btrfs >/dev/null || { report "the btrfs command is missing"; exit 0; }
