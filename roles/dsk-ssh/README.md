...
flowchart LR
    subgraph deps [Dependencies]
        dep_dsk_git["dsk-git 💻 ⚙️"]
    end
    subgraph role [dsk-ssh 💻]
        svc_ssh["ssh"]
    end
    dep_dsk_git -- "1:1" --> svc_ssh
```
...
- **Clones a remote SSH config repository** into `~/.ssh` using the `dsk-git` role.
...
cd core
make onboard
make compose-deploy mode=reinstall apps=dsk-ssh full_cycle=false
```
