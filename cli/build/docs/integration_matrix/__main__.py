...
REPORT = PROJECT_ROOT / "docs" / "requirements" / "027-integration-matrix.md"
...
def scan_framework_edges():
    ...
    for prefix in ("web-app-", "web-svc-"):
        for path in sorted(ROLES_DIR.glob(f"{prefix}*/{ROLE_FILE_META_SERVICES}")):
            ...
            for line in lines:
                ...
                if target and target != entity and target not in seen:
                    seen.add(target)
                    rel = f"../../roles/{role}/{ROLE_FILE_META_SERVICES}"
...
> Generated by `make integration-matrix`. Do not edit by hand, edit the generator's `EDGES` map and re-run.
