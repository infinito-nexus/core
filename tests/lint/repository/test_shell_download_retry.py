...
_CURL_OUT_RE = re.compile(r"(?:-o|--output)\s+(?!/dev/null\b|-\s)(\S+)|(?:-O\b|--remote-name\b)")
        ...
        for glob in _GLOBS:
            for path in sorted(PROJECT_ROOT.glob(glob)):
                ...
                for no, line in enumerate(lines, start=1):
                    bad = None
                    if _CURL_RE.search(line) and _CURL_OUT_RE.search(line):
                        if "--retry-all-errors" not in line:
                            bad = "curl download without --retry-all-errors"
                    elif _WGET_RE.search(line) and not _WGET_STDOUT_RE.search(line):
                        if "--tries=" not in line and "-t " not in line:
