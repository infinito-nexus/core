...
# Mount-target prefixes that are an engine's on-disk data directory (NFS-hostile).
...
# Engine keywords matched anywhere in the target path.
    ...
    def test_unpinned_roles_declare_no_engine_data_volume(self) -> None:
        ...
        for volumes_yml in sorted(roles_root.glob(f"*/{ROLE_FILE_META_VOLUMES}")):
            ...
            if (
                ...
                continue  # pinned: volumes are node-local, engine data is safe
            try:
